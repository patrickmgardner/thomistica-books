...
def should_wrap(segment, first):
    ...
    # preserve the first segment, which often contains the top nav link and <hr>
    if first:
        return False

    # leave alone segments that are only structural blocks
    if BLOCK_ONLY_RE.fullmatch(stripped):
        return False

    # if, after comments/anchors, the segment begins with a block tag, leave it alone
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python normalize_books_html.py file1.htm [file2.htm ...]")
        return
